...
- Subtask 6 (Settings UI for floating-box scale + layout mode) done. Added a 60%-100% range slider (step 5%) bound to `settings.floatingBoxScale` with a live percentage label, and a Spread/Compact radio group bound to `settings.layoutMode`, in `SettingsModal.tsx`'s General tab, both persisting via the existing `updateSettings` path. New `.settings-slider-row`/`.settings-radio-group` CSS added to `SettingsModal.css`. Fixed: reviewer caught the slider's label/thumb were bound to the raw unclamped `floatingBoxScale`, which could disagree if a persisted value ever fell outside `[0.6, 1]`; fixer added a `clamp` helper applied to both the label and the input's `value` prop. Nothing yet consumes these settings to affect the actual layout (separate later subtask). Remaining: minor a11y polish item noted but not fixed — the slider's explicit `aria-label` overrides its wrapping `<label>`'s implicit name, so screen readers don't announce the live percentage.
- Subtask 7 (Apply floating-box scale + layout mode to the shell layout) done. Added a `data-app-layout` attribute + `--app-box-scale` CSS variable, set from `AppSettings` in a new `App.tsx` effect mirroring the existing theme-attribute pattern. `.app-shell-nav`/`.app-shell-main` both get `transform: scale(var(--app-box-scale, 1))`; new `:root[data-app-layout='compact']` rules center the shell and cap `.app-shell-main`'s width instead of growing to fill. Fixed: reviewer caught the initial corner-anchored `transform-origin` values only shrank each box toward its inner-facing side (never revealing outer-edge dot-grid) and made the compact-mode gap grow unpredictably with scale; fixer changed both to `top center` for symmetric shrinking. Also fixed: `--app-box-scale` was written from the raw unclamped settings value even though only the settings-modal display was clamped; fixer added an inline clamp to `[0.6, 1]` in the effect. Remaining: `transform` on these containers changes the containing block for any `position: fixed` descendant — none exist today (the one positioned element inside, `.resume-tool-sidebar`, uses `sticky`), but noted as a footgun for future additions.
- Subtask 8 (Give the resume editor's right sidebar its own floating box) done. `AppShell.css` gained `.app-shell-editor-sidebar`, a third floating box (same chrome/scale as nav/main) rendered as a sibling only while the deep editor is open. Fixed (major): the first implementation pushed the sidebar's full JSX up via a `useEffect` + callback with a hand-picked dependency array; reviewer caught real stale-closure data loss (handlers closed over `state`/`appSettings` wholesale while deps only tracked narrowed fields) and per-keystroke render lag (inputs got an extra deferred render pass). Fixer replaced the mechanism entirely with a React portal — `ResumeTool` now computes the sidebar JSX directly in its own render body (no effect) and portals it via `createPortal` into a DOM node owned by `AppShell`, threaded through `ResumeMakerScreen` via `editorSidebarNode`/`onEditorSidebarOpenChange`; only a stable boolean "is the editor open" signal crosses an effect boundary. Re-review confirmed both bugs are gone and the portal lifecycle (mount/typing/unmount) is correctly ordered with no flash or teardown errors. Sidebar content unchanged. No remaining concerns.
- Subtask 9 (Move "← Back to variations" into the left nav rail) done. `ResumeTool.tsx` now receives `onNavContextChange` (forwarded from `ResumeMakerScreen.tsx`) and pushes a single Back button into the left nav rail via a `useEffect`, clearing it on unmount; removed from the sidebar JSX entirely, everything else there unchanged. Removed the now-orphaned `.resume-tool-back-btn` CSS rule. Fixed (major): reviewer caught a real infinite render loop — the effect initially depended on `[onBack, onNavContextChange]`, but `onBack` is a non-memoized inline closure recreated every `ResumeMakerScreen` render, and since `onNavContextChange` triggers an `AppShell` re-render (re-rendering the non-memoized `ResumeMakerScreen`, producing a new `onBack`), this created an unbounded loop pegging the CPU while the editor was open. Fixer removed `onBack` from the dependency array (safe since `openSchema.id` can't change without unmounting `ResumeTool` first), matching an established convention elsewhere in the codebase, and verified the added eslint-disable comment isn't a no-op. No remaining concerns.
- Subtask 10 (Default Job Title from the resume's header tagline) done. `ResumeMakerScreen.tsx`'s `handleNewSchema` now defaults `jobTitle` to `defaultResume.header.tagline`; `ResumeTool.tsx`'s `handleImportFile` now defaults it to `resume.header.tagline`. One-time default at creation time only; Job Title stays independently editable afterward. Reviewer confirmed `Header.tagline` is always a real string (never `undefined`/`null`) across every resume-producing path, and confirmed `handleNewVariation`/`handleAddVariation`/`handleApplySuggestions` were untouched, still inheriting from a source variation. No fixer pass needed. Remaining: an accepted, in-scope side effect — a brand-new schema's auto-filled variation name will incorporate the tagline placeholder text until the user overwrites Job Title, a direct consequence of this subtask's request.
- Subtask 11 (Fix PDF export white border (full bleed)) done. `exportPdf.tsx` — removed the `MARGIN_PT` constant entirely; `maxWidth`/`maxHeight` now use the full page dimensions instead of page-minus-margins; `y` is now centered like `x` instead of pinned to the old margin offset. Reviewer confirmed the math is safe (no negative `x`/`y`) and no stale margin references remain. No fixer pass needed. Remaining: for typical multi-section resume content (taller than the page's aspect ratio at the fixed 900px capture width), the existing "scale to fit height, preserve aspect ratio" logic still leaves horizontal whitespace bars — narrower than before, but not eliminated, since it's driven by aspect-ratio mismatch rather than the margin. True edge-to-edge fill for that case would require stretching or cropping the image, a distinct design decision not covered by this subtask.
