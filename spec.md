...
- Subtask 4 (Landing page + schema/variation navigation) done. Added `LandingScreen.tsx` (Resume Maker vs. disabled "Other Utilities" placeholder) and `ResumeMakerScreen.tsx` (schema grid → variation grid with favorites row and star toggles → editor); `App.tsx` now drives real navigation state instead of the previously-unused `screens` map. `ResumeTool.tsx` gained one required `onBack` prop for a small "← Back to variations" control, no other internal restructuring. Fixed: a critical data-loss bug where "+ New variation" persisted the new variation then immediately overwrote it via a stale-closure chain into the variation-select handler, losing the new variation and leaving `activeVariationId` dangling — fixer inlined the select-and-navigate logic against the already-fresh state. Reviewer confirmed the analogous "+ New schema" path was not affected by the same bug class. Remaining: schema deletion not implemented (not requested); manual click-through in a real browser recommended before relying on this heavily.
- Subtask 5 (Sidebar + tabs shell in the editor) done — pure UI relocation, no handler-logic changes. `ResumeTool.tsx` restructured into a persistent sidebar (Back button + "Files"/"AI-tailored" tabs) beside a main content area for `<ResumePreview>`; print-hiding CSS consolidated from four separate selectors down to one (`.resume-tool-sidebar`) since all editing chrome now lives inside it. Reviewer found no bugs — no fixer pass needed. Manual click-through in a real browser recommended before relying on this heavily (implementor worked headless).
- Subtask 6 (Files tab: combined auto-fill variation name) done. Added `buildAutoVariationName` in `filename.ts` (`"<firstName> <lastName> - <jobTitle>"`); Name field is read-only when `autoFillVariationName` is on, kept in sync inline; Job title stays always-editable. Fixed 4 reviewer findings: the resync effect never fired on mount when auto-fill was already on (the default) since its ref was seeded with the current value instead of `false`; new variations from "+ New variation" or applying AI suggestions bypassed the auto-fill format entirely; the resync effect only fixed the active variation, not the whole schema; `buildAutoVariationName` could silently return an empty string (blank dropdown entry) — now falls back to `'Untitled'`. No remaining concerns.
- Subtask 7 (Files tab: export-on-select) done. `handleExport` reparameterized to take `format: ExportFormat` directly instead of reading closure state; removed the standalone Export button; the format `<select>`'s `onChange` now triggers export immediately. Reviewer flagged that native `<select>` fires `onChange` on keyboard arrow-key navigation too (not just deliberate clicks), which could trigger accidental exports/print dialogs — but this is the literal UX both planning.md and spec.md asked for (dropdown-triggers-export, no button), so left as-is per the explicit spec rather than unilaterally redesigning it. Also noted: reselecting the same already-selected format won't retrigger export (accepted tradeoff, not a bug). Flagging both for awareness, not blocking.
- Subtask 8 (Emoji picker for section/contact icons) done. Added `emojiPicker/EmojiPicker.tsx` (curated popover) and optional `SectionIcons`/`ContactItem.icon` fields on `Resume` (correctly optional — old resumes still validate). Every section-header/contact icon is now a real `<button>` with proper `aria-label` (replacing decorative `aria-hidden` spans); `exportHtml.ts`/`importHtml.ts` updated so customized icons round-trip through HTML export/import. Fixed: a toggle-close race where clicking an already-open icon's own trigger never closed the popover (outside-click handler didn't know about the trigger's bounds) — added a `triggerRef` the close handler also checks, and extracted a `ContactIconTrigger` component for per-instance refs. Remaining (not fixed, low severity, by design): `exportHtml.ts`'s `DEFAULT_SECTION_ICON` is a second independently-maintained copy of `ResumePreview.tsx`'s `SECTION_ICON` — a future-drift risk, not a current bug.
- Subtask 9 (Fix: PDF export via a real PDF-generation library) done. Added `html2canvas`/`jspdf` deps and `exportPdf.ts`'s `buildResumePdfBlob` (captures the resume DOM at 2x scale, embeds as a single-page US Letter PDF, scales down rather than clipping if content overflows). A parallel `.pdf-export-mode` CSS ruleset duplicates the existing `@media print` block's intent for `html2canvas`'s offscreen clone (via `onclone`, no flash in the live UI, since real print media queries never fire during canvas capture). `ResumePreview` exposes its root via a new optional `rootRef` prop; the PDF export branch now uses this instead of `window.print()`. Reviewer checked all the risky spots (compound vs. descendant CSS selector correctness, `onclone` timing, theme-controls exclusion from capture, ref typing) and found no bugs — no fixer pass needed. Remaining: rasterized/non-selectable text and single-page-only are accepted, deliberate trade-offs, not gaps; manual smoke test of the actual PDF download recommended (implementor worked headless).
- Subtask 10 (Fix: AI-assisted structured DOCX/PDF import) done — final subtask in this spec. Added `aiImport.ts`'s `structureResumeFromText`, using the same Anthropic fetch/error-handling pattern as `aiTailor.ts`; once any JSON is parsed, a tolerant `coerceResume` always produces a fully-valid `Resume` with fresh ids, never throwing on malformed/partial data. `importDocx.ts`/`importPdf.ts` attempt this first when a key+model are configured, silently falling back to the existing heuristic on any failure; HTML import untouched (already structured). Reviewer tried to construct inputs that would break the "never throws once parsed" guarantee and found none — no fixer pass needed. Remaining: one trivial non-blocking note (AI-attempt gate checks truthiness, not trimmed-emptiness, so a whitespace-only key wastes one network round-trip before falling back correctly anyway).

All 10 subtasks in this spec are now complete.
